        ...
        foreach ($orders as $order) {
            ...
            if (empty($ward_code)) {
                ...
                $errors[] = sprintf(
                    ...
                    __('Order #%d: missing address data', 'onestudio-vietnam-address-for-woocommerce'),
                    $order_id
                );
                continue;
            }
            ...
            if ($resolution['status'] === 'matched') {
                ...
                $errors[] = sprintf(
                    /* translators: %d: order ID. */
                    __('Order #%d: multiple possible new wards, needs manual review', 'onestudio-vietnam-address-for-woocommerce'),
                    $order_id
                );
            } else {
                $failed++;
                $errors[] = sprintf(
                    /* translators: %d: order ID. */
                    __('Order #%d: no matching ward found in the conversion table', 'onestudio-vietnam-address-for-woocommerce'),
                    $order_id
                );
            }
        }
