...
- **Optional central data server** — point the plugin at a central sync server (self-hostable, see [vn-address-api-server](https://github.com/jungdevtoday/vn-address-api-server)) to receive administrative boundary updates without a plugin update. Falls back to the bundled data automatically if unset or unreachable.
- **Multilingual** — Vietnamese, English, Français, Deutsch, 日本語.

## Installation

**From WordPress.org** (recommended once approved): search for "Vietnam Address for WooCommerce" under Plugins → Add New.

**Manual install:**

1. Download the latest `vietnam-address-for-woocommerce.zip` from [Releases](../../releases).
...
[vn-address-api-server](https://github.com/jungdevtoday/vn-address-api-server) — the open-source central data server this plugin can optionally sync with. Self-hostable if you'd rather not depend on the default one.
